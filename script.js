...
audio.addEventListener("timeupdate", () => {
	...
	document.querySelectorAll(".thumb").forEach((element) => {
		element.style.left =
			(audio.currentTime / audio.duration) * 100 - 1.5 + "%";
...
document.querySelectorAll(".seekbar").forEach((element) => {
	element.addEventListener("click", (e) => {
		...
		audio.currentTime =
			(audio.duration *
				Math.floor((e.offsetX / element.offsetWidth) * 100)) /
			100;
	});
});
...
async function playSong(playlist, song, element) {
	...
	if (
		songName.substring(songName.length - 4).toLowerCase() == ".jpg" ||
		songName.substring(songName.length - 4).toLowerCase() == ".png" ||
		songName.substring(songName.length - 4).toLowerCase() == ".svg" ||
		songName.substring(songName.length - 4) == ".gif" ||
		songName.substring(songName.length - 5).toLowerCase() == ".jepg" ||
		songName.substring(songName.length - 5).toLowerCase() == ".webp" ||
		songName.substring(songName.length - 4).toLowerCase() == ".avif"
	) {
...
audio.onended = async () => {
	...
	if (Array.from(document.querySelectorAll(".songCard")).indexOf(currCard) <
		Array.from(document.querySelectorAll(".songCard")).length - 2) {
		playSong(
			playlist,
			songs[Number(currCard.dataset.sindex) + 1],
			document.querySelectorAll(".songCard")[
				Number(currCard.dataset.sindex) + 1
			],
		);
		displayCurrSong(
			playlist,
			songs[Number(currCard.dataset.sindex) + 1],
			library.indexOf(playlist),
		);
	}
};
document.querySelector(".prev").addEventListener("click", async () => {
	...
	let songs = await getSongs(currCard.dataset.pl);
	if (
		Array.from(document.querySelectorAll(".songCard")).indexOf(currCard) > 0
	) {
		let prevEle = Array.from(document.querySelectorAll(".songCard"))[
			Array.from(document.querySelectorAll(".songCard")).indexOf(
				currCard,
			) - 1
		];
		...
		displayCurrSong(
			playlist,
			songs[Number(prevEle.dataset.sindex)],
			library.indexOf(playlist),
		);
	} else {
		audio.currentTime = 0;
...
document.querySelector(".next").addEventListener("click", async () => {
	...
	if (
		Array.from(document.querySelectorAll(".songCard")).indexOf(currCard) <
		Array.from(document.querySelectorAll(".songCard")).length - 2
	) {
        let prevEle;
        if(Array.from(document.querySelectorAll(".songCard")).indexOf(currCard)==-1){
           prevEle = Array.from(document.querySelectorAll(".songCard"))[
			Array.from(document.querySelectorAll(".songCard")).indexOf(
				currCard,
			) + 2
		]; 
	...
	if (plapau.src.includes("assets/play.svg")) {
		...
		if (audio.src) {
			...
			for (let j = 0; j < songs.length; j++) {
				const element = songs[j];
				if (
					element.substring(element.length - 4).toLowerCase() ==
						".mp3" ||
					element.substring(element.length - 4).toLowerCase() ==
						".mp4"
				) {
			...
			// console.log(`library/${library[0]}/${abc}`);
			// console.log(audio.src)
			if (audio.src.includes(encodeURI(`library/${library[0]}/${abc}`))) {
				currCard = document.querySelectorAll(".songCard")[0];
				// console.log("Gdfgd")
				currCard.style.backgroundColor = "#1f1f1f";
			}
			// console.log("fsdfsdf")
			...
			playSong(
				library[0],
				songs[0],
				document.querySelectorAll(".songCard")[0],
			);
		}
	}
});
...
async function displayCurrSong(playlist, song, indexForCapLib) {
	...
	songs.forEach((songName) => {
		if (
			...
			hasCover = true;
			document.querySelector(".currSong").innerHTML =
				`<div class="songCard">
	...
	if (!hasCover) {
		...
		document.querySelectorAll(".randBack2")[
			document.querySelectorAll(".randBack2").length - 1
		].style.background = Array.from(
			document.querySelectorAll(".playlistCard"),
		)[indexForCapLib].querySelector(".randBack").style.background;
	}
}
async function main() {
	// console.log(library);
	...
	for (let index = 0; index < library.length; index++) {
		...
		for (let indextemp = 0; indextemp < songs.length; indextemp++) {
			...
			// songs.forEach(songName => {
			if (
				songName.substring(songName.length - 4).toLowerCase() ==
					".jpg" ||
				songName.substring(songName.length - 4).toLowerCase() ==
					".png" ||
				songName.substring(songName.length - 4).toLowerCase() ==
					".svg" ||
				songName.substring(songName.length - 4) == ".gif" ||
				songName.substring(songName.length - 5).toLowerCase() ==
					".jepg" ||
				songName.substring(songName.length - 5).toLowerCase() ==
					".webp" ||
				songName.substring(songName.length - 4).toLowerCase() == ".avif"
				...
				document.querySelector(".library").innerHTML +=
					`<div class="playlistCard"><img  src="library/${library[index]}/${songName}" alt="" class="cover">
					...
					songs.forEach((element) => {
						if (
							...
							document.querySelector(".songs").innerHTML +=
								`<div class="songCard" data-sIndex="${sIndex}"
							...
							if (!firstSongReady) {
								audio.src = `library/${library[0]}/${element}`;
								firstSongReady = true;
							}
						}
		...
		if (!hasCover) {
			...
			document.querySelector(".library").innerHTML +=
				`<div class="playlistCard"><div class="randBack"><img src="assets/music_note_2_24dp_000000_FILL0_wght400_GRAD0_opsz24.svg" alt="" class="cover"></div>
                    <div class="plName">${capLib[index]}</div>
                </div>`;
			document.querySelectorAll(".randBack")[
				document.querySelectorAll(".randBack").length - 1
			].style.background =
				`linear-gradient(135deg,rgb(${prim1},${prim2},${prim3}),rgb(${sec1},${sec2},${sec3}))`;
			if (index == 0) {
				let sIndex = 0;
				document.querySelector(".playll").innerHTML = capLib[0];
				let currSongImageRendered = false;
				songs.forEach((element) => {
					if (
						...
						document.querySelector(".songs").innerHTML +=
							`<div class="songCard"  data-pl="${library[index]}" data-sIndex="${sIndex}"><div class="back2 randBack">
                            <img src="assets/music_note_2_24dp_000000_FILL0_wght400_GRAD0_opsz24.svg"  alt="">
                            </div>
                            <div class="info">
                            <div class="songName">${element.slice(0, element.lastIndexOf("."))}
						...
						// console.log(document.querySelector(".songCard").dataset.pl)
						document.querySelectorAll(".back2")[
							document.querySelectorAll(".back2").length - 1
						].style.background =
							`linear-gradient(135deg,rgb(${prim1},${prim2},${prim3}),rgb(${sec1},${sec2},${sec3}))`;
						if (!currSongImageRendered) {
                        ...
                        <img src="assets/music_note_2_24dp_000000_FILL0_wght400_GRAD0_opsz24.svg" alt="">
                         ...
                         <div class="songName">${element.slice(0, element.lastIndexOf("."))}</div>
							...
							document.querySelectorAll(".randBack2")[
								document.querySelectorAll(".randBack2").length -
									1
							].style.background =
								`linear-gradient(135deg,rgb(${prim1},${prim2},${prim3}),rgb(${sec1},${sec2},${sec3}))`;
	...
	for (
		let indexOfSong = 0;
		indexOfSong < document.querySelectorAll(".songCard").length;
		indexOfSong++
	) {
		const element = document.querySelectorAll(".songCard")[indexOfSong];
		// i++;
		...
		element.addEventListener("click", async () => {
			playSong(
				library[0],
				songs[Number(element.dataset.sindex)],
				element,
			);
	...
	for (
		let index = 0;
		index < Array.from(document.querySelectorAll(".playlistCard")).length;
		index++
	) {
		const element = Array.from(document.querySelectorAll(".playlistCard"))[
			index
		];
		element.addEventListener("click", async () => {
			// for (let index = 0; index < library.length; index++) {
					...
					songs.forEach((element) => {
						if (
							...
							document.querySelector(".songs").insertAdjacentHTML(
								"beforeend",
								`<div class="songCard" data-sIndex="${sIndex}" data-pl="${library[index]}">
                        <img src="library/${library[index]}/${songName}" alt="">
                        <div class="info">
                        <div class="songName">${element.slice(0, element.lastIndexOf("."))}
                        </div>
			...
			for (
				...
				const element =
					document.querySelectorAll(".songCard")[indexOfSong];
				element.addEventListener("click", async () => {
					playSong(
						element.dataset.pl,
						songs[Number(element.dataset.sindex)],
					...
					displayCurrSong(
						element.dataset.pl,
						songs[Number(element.dataset.sindex)],
						library.indexOf(element.dataset.pl),
					);
				});
			}
			...
			if (!hasCover) {
				...
				songs.forEach((element) => {
					if (
						...
						document.querySelector(".songs").innerHTML +=
							`<div class="songCard" data-sIndex="${sIndex}" data-pl="${library[index]}"><div class="back2 randBack">
                            <img src="assets/music_note_2_24dp_000000_FILL0_wght400_GRAD0_opsz24.svg"  alt="">
                            </div>
                            <div class="info">
                            <div class="songName">${element.slice(0, element.lastIndexOf("."))}
					...
					document.querySelectorAll(".back2")[
						...
						document.querySelectorAll(".playlistCard"),
					)[index].querySelector(".randBack").style.background;
					if (!currSongImageRendered) {
						// document.querySelector(".randBack2").style.
						document.querySelector(".currSong").innerHTML =
							`<div class="songCard">
			...
			setTimeout(() => {
				playSong(
					library[index],
					songs[0],
					document.querySelector(".songCard"),
				);
			}, 200);
		});
	}
